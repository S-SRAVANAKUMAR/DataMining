...
rmse_results = {'UserCF': [], 'ItemCF': []}

# Evaluate RMSE for each K value
for k in k_values:
    pred_user_cf = user_based_cf(R, user_id, k=k)  # User CF
    pred_item_cf = item_based_cf(R, user_id, k=k)  # Item CF
    
    _, rmse_user = evaluate(true_ratings, pred_user_cf)
    _, rmse_item = evaluate(true_ratings, pred_item_cf)
    
    rmse_results['UserCF'].append(rmse_user)
    rmse_results['ItemCF'].append(rmse_item)

# Find the best K for UserCF and ItemCF
best_k_user = k_values[np.argmin(rmse_results['UserCF'])]
best_k_item = k_values[np.argmin(rmse_results['ItemCF'])]

print(f"Best K for User-based CF: {best_k_user}")
print(f"Best K for Item-based CF: {best_k_item}")

# Plot the RMSE values for each K
plt.plot(k_values, rmse_results['UserCF'], label='UserCF', marker='o')
plt.plot(k_values, rmse_results['ItemCF'], label='ItemCF', marker='o')
plt.xlabel('Number of Neighbors (K)')
plt.ylabel('RMSE')
